...
*/
      ...
      if (CUR_USERID != null) {
        try {
          ...
          var parameter = {
            ...
            PRODUCT_VARIENT_ID: model.prVarientList![model.selVarient!].id,
            QTY: qty,
             "image": fileDate[1] == null ? null : fileDate[1].toString()
          };
           print("aaaaaaaaaaaaaaaaaaaaaaaa====${parameter.toString()}");
          Response response =
              await post(manageCartApi, body: parameter, headers: headers)
                  .timeout(Duration(seconds: timeOut));
          print("SYREYEYEET======${Response}");
  ...
  _addOn(){
    return ListView.builder(
      shrinkWrap: true,
      itemCount:AddOnNew!.data!.length,
      physics:  NeverScrollableScrollPhysics(),
      itemBuilder: (context, index) {
        return  Padding(
          padding: const EdgeInsets.all(8.0),
           ...
           child: Row(
             children: [
               Container(
                 ...
                 child: ClipRRect(
                   borderRadius: BorderRadius.circular(10),
                   child: Image.network("$imageUrl${AddOnNew!.data![index].image}",fit: BoxFit.fill,)
                 ),
               ),
               Padding(
                 ...
                 child: Column(
                   crossAxisAlignment: CrossAxisAlignment.start,
                   children: [
                     Text("${AddOnNew!.data![index].name}"),
                     Text("$CUR_CURRENCY${AddOnNew!.data![index].price}")
                   ],
                 ),
               ),
